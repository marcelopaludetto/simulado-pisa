...
function ActivityProgress({ answered }: { answered: number }) {
  return (
    <section className="activity-progress" aria-label={`${answered} de ${questions.length} atividades concluídas`}>
      <div className="progress-heading"><span>ATIVIDADES</span><strong>{answered}/{questions.length}</strong></div>
      <div className="progress-dots">
        {questions.map((item, index) => <span key={item.id} className={index < answered ? 'complete' : ''} aria-hidden="true">{index < answered ? '✓' : ''}</span>)}
      </div>
    </section>
  );
}
...
function Dot() { return <span className="inline-dot" role="img" aria-label="bolinha" />; }
...
function QuestionBody({ id, answer, setAnswer }: { id: number; answer: Answer; setAnswer: (v: Answer) => void }) {
  if (id === 1) return <><p className="prompt">A bola <Dot /> se move uma casa por comando. Qual sequência leva a bola até a casa verde?</p><div className="stimulus"><MiniGrid /><RadioCards value={answer} onChange={setAnswer} options={[{ value: 'A', label: '↑  ←  ↑  →' }, { value: 'B', label: '↑  ↑  ←  ↓' }, { value: 'C', label: '↑  ←  ↑  ←' }, { value: 'D', label: '←  ←  ↑  →' }]} /></div></>;
  if (id === 2) return <><p className="prompt">O programa deve levar a bola <Dot /> até a casa verde sem passar pelas casas pretas. Qual comando está faltando?</p><div className="stimulus"><div className="program"><Block tone="event">início</Block><Block>mover para a esquerda</Block><Block>mover para cima</Block><Block>mover para cima</Block><Block tone="repeat">repetir 3 vezes</Block><span className="indent"><Block>?</Block></span><Block>mover para baixo</Block><Block>mover para baixo</Block></div><MiniGrid rows={4} cols={4} start={14} goal={16} blocked={[4,10,11,15]} /></div><RadioCards value={answer} onChange={setAnswer} options={['cima','baixo','esquerda','direita'].map(v => ({ value: v, label: `Mover para ${v}` }))} /></>;
  ...
  if (id === 4) return <><p className="prompt">Execute mentalmente o programa. Clique na casa em que a bola <Dot /> terminará.</p><div className="stimulus"><div className="program"><Block tone="event">início</Block><Block>mover para a direita</Block><Block tone="repeat">repetir 3 vezes</Block><span className="indent"><Block tone="condition">se a bola estiver em uma casa cinza</Block><span className="indent"><Block>mover para baixo</Block></span><Block tone="condition">senão</Block><span className="indent"><Block>mover para a direita</Block></span></span></div><MiniGrid rows={6} cols={6} start={1} goal={0} clickable selected={typeof answer === 'number' ? answer : null} onSelect={setAnswer} /></div></>;
...
export default function Home() {
  ...
  const assessmentResult = useMemo(() => calculateResult(questions.map(item => ({ id: item.id, answer: answers[item.id], correct: isCorrectAnswer(item.id, answers[item.id]) }))), [answers]);
  ...
  if (submitted) return <main className="result-page"><a href="https://www.startalura.com.br/"><img className="result-logo" src="/figma/logo-start.svg" alt="Start" /></a><section className="result-card"><img className="cody-feedback" src="/figma/cody-feedback.svg" alt="Cody comemorando o resultado" /><p className="eyebrow">Resultado na escala de proficiência</p><h1>{assessmentResult.score} pontos de escala</h1><div className="level-summary"><strong>Nível {assessmentResult.level}</strong><span>Intervalo estimado: {assessmentResult.ci95[0]}–{assessmentResult.ci95[1]}</span></div><p className="result-lead">{assessmentResult.atOrAboveBaseline ? 'Você atingiu o nível básico de proficiência desta avaliação.' : 'Seu resultado ainda está abaixo do nível básico de proficiência desta avaliação.'}{assessmentResult.levelUncertain ? ' O intervalo de confiança atravessa mais de um nível.' : ''}</p><div className="score-track scale-track"><i style={{left:`${Math.max(0,Math.min(100,(assessmentResult.score-300)/5))}%`}} /></div><div className="review-list">{questions.map(item => { const measured = assessmentResult.credits.find(result => result.id === item.id); const thresholds = itemDifficulties[item.id]; const achievedDifficulty = measured?.credit ? thresholds[measured.credit - 1] : null; const targetDifficulty = thresholds[thresholds.length - 1]; return <button key={item.id} className={`result-item ${measured?.correct ? 'correct' : measured?.credit ? 'partial' : 'incorrect'}`} onClick={() => {setCurrent(item.id-1); setSubmitted(false)}}><img src={numberAsset(item.id, measured?.correct ? 'Concluído' : 'Padrão')} alt={`Atividade ${item.id}`} /><span><strong>{item.title}</strong><small>Dificuldade do item: {targetDifficulty} · Nível {proficiencyLevel(targetDifficulty)}</small></span><span>{measured?.correct ? `Certo · dificuldade alcançada: ${achievedDifficulty}` : measured?.credit ? `Parcial · dificuldade alcançada: ${achievedDifficulty}` : `Errado · dificuldade abaixo de ${thresholds[0]}`}</span></button> })}</div><p className="scale-note">Escala ancorada nas dificuldades declaradas dos itens. As pontuações não são comparáveis às do PISA da OCDE.</p><button className="primary-button" onClick={reset}><span aria-hidden="true">↻</span> Refazer simulado</button></section></main>;
  return <main className="app-shell"><aside className="sidebar"><div className="brand"><a href="https://www.startalura.com.br/"><img src="/figma/logo-start.svg" alt="Start" /></a></div><ActivityProgress answered={answered} /><nav aria-label="Questões do simulado">{questions.map((item,index)=>{const done=hasAnswer(item.id, answers[item.id]); const state=index===current?'Padrão':done?'Concluído':'Inativo'; return <button key={item.id} className={index===current?'active':''} onClick={()=>setCurrent(index)}><img src={numberAsset(item.id,state)} alt="" /><span><strong>{item.stage}</strong><small>{item.title}</small></span></button>})}</nav></aside><section className="workspace"><div className="mobile-topbar"><span className="mobile-brand"><a href="https://www.startalura.com.br/"><img src="/figma/logo-start.svg" alt="Start" /></a></span><ActivityProgress answered={answered} /></div><header><div><p className="eyebrow">{q.stage} · questão {q.id} de 8</p><h1>{q.title}</h1></div><div className="points">Dificuldade {itemDifficulties[q.id].at(-1)}</div></header><div className="question-card"><div className="skill-chip">Habilidade <strong>{q.skill}</strong></div><QuestionBody id={q.id} answer={answers[q.id] ?? null} setAnswer={setAnswer} /></div><footer><button className="secondary-button" disabled={current===0} onClick={()=>setCurrent(v=>v-1)}><span aria-hidden="true">←</span> Anterior</button>{current < questions.length-1 ? <button className="primary-button" onClick={()=>setCurrent(v=>v+1)}>Próxima <span aria-hidden="true">→</span></button> : <button className="primary-button finish" disabled={answered<8} onClick={()=>setSubmitted(true)}>Finalizar avaliação <span aria-hidden="true">›</span></button>}</footer></section></main>;
}
