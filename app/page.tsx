...
function QuestionBody({ id, answer, setAnswer }: { id: number; answer: Answer; setAnswer: (v: Answer) => void }) {
  if (id === 1) return <><p className="prompt">Cada seta move <Dot /> uma casa na direção indicada pela seta. Qual sequência de setas leva <Dot /> até a casa verde?</p><div className="stimulus"><MiniGrid /><RadioCards value={answer} onChange={setAnswer} options={[{ value: 'A', label: '↑  ←  ↑  →' }, { value: 'B', label: '↑  ↑  ←  ↓' }, { value: 'C', label: '↑  ←  ↑  ←' }, { value: 'D', label: '←  ←  ↑  →' }]} /></div></>;
  if (id === 2) return <><p className="prompt">Cada comando move <Dot /> uma casa na direção indicada. O programa deve levar <Dot /> até a casa verde sem passar pelas casas pretas. Qual comando está faltando?</p><div className="stimulus"><div className="program"><Block tone="event">início</Block><Block>mover para a esquerda</Block><Block>mover para cima</Block><Block>mover para cima</Block><Block tone="repeat">repetir 3 vezes</Block><span className="indent"><Block>?</Block></span><Block>mover para baixo</Block><Block>mover para baixo</Block></div><MiniGrid rows={4} cols={4} start={14} goal={16} blocked={[4,10,11,15]} /></div><RadioCards value={answer} onChange={setAnswer} options={['cima','baixo','esquerda','direita'].map(v => ({ value: v, label: `Mover para ${v}` }))} /></>;
